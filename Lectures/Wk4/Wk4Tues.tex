...
\item Not all functions take arguments and/or provide return values
...
\item Do the following:
	\begin{enumerate}
		...
		\item Write a \textit{function prototype}
			...
			\end{itemize}
		\pause
		\item Somewhere below \texttt{main()} (or in another \texttt{.c} file) write the function
	\end{enumerate}
\pause
...
\frametitle{Writing Functions - Example}
\begin{itemize}
\item In mathematics, calculate $\sqrt{X}$ by iterating:
\begin{equation*}
...
\begin{lstlisting}[style=CStyle]
// Calculate sqrt(x)
float X = 26; // Test value, sqrt(26)=5.0990
