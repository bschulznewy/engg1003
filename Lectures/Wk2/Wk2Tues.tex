...
\item Lab experience will hopefully rebuild your confidence
	\begin{itemize}
		\item Ideally, before the HECS census date
		\item I have 2 labs to do this ... oh crap
	\end{itemize}
\end{itemize}
\end{frame}
...
\item Instead, we can use a more modern standard:
	\begin{itemize}
		\item $1024$ bytes = 1 kibiByte (KiB)
		\item $1024^2$ bytes = 1 Mebibyte (MiB)
		\item $1024^3$ bytes = 1 Gibibyte (GiB)
		\item ...etc
	\end{itemize}
\item Generally speaking:
	\begin{itemize}
		\item KB (etc) implies powers of two to \textit{engineers}
		\item KB (etc) implies powers of ten to \textit{marketing}
			\begin{itemize}
