...
\item This sequence interesting properties, eg:
	\begin{itemize}
		...
		\item This has applications in art. eg: 16:10 screen ratio is 1.6
	\end{itemize}
\end{itemize}
\end{frame}
...
\frametitle{Fibonacci Sequence}
\begin{itemize}
\item Mathematically, we can write this as a list of numbers, $x_1, x_2, x_3, ... , x_n$, where:
\begin{equation}
x_n = x_{n-1} + x_{n-2}
\end{equation}
and:
...
\begin{frame}
\frametitle{Sequences in General}
\begin{itemize}
...
\item All 3 variables change, the sequence in which they change is \underline{\textbf{crucial}}:
	\begin{lstlisting}[style=CStyle]
...
\item There are a few solutions:
	\begin{itemize}
		...
		\item Do calculation as \texttt{unsigned int} and compare with \texttt{INT\_MAX}
	\end{itemize}
\end{itemize}
\end{frame}
...
\frametitle{Fibonacci Sequence}
\begin{itemize}
\item If overflow occurs when using \texttt{int} then the result of a calculation which should be positive will be negative
...
int main() {
	...
	int xN;
	int n = 1;
	while((unsigned int)xNm1 + (unsigned int)xNm2 < 2147483647u) {
		xN = xNm1 + xNm2;
		printf("%d\n", xN);
		n++;
		xNm2 = xNm1;
