...
This lab covers C \textit{functions}, \texttt{static} variables, and arrays

If I have time the introduction will be expanded with examples, etc. Apologies if this doesn't happen.
...
Your program should ``input'' the data via an array initialisation. This is to avoid having to manually type test data into \texttt{scanf()} multiple times while debugging.
\\~\\
...
f(x) = e^{x^2}
\end{equation*}
as a C function and uses the trapezoidal rule to numerically calculate its integral between two given points: $x_1$ and $x_2$ (assume $x_1 < x_2$).
\\~\\
