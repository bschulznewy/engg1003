...
So, it is Page \thepage{} of these notes and we haven't done anything \textit{useful} yet! Ok, lets start doing some data processing.
...
These are all \textit{binary} operators, meaning they operate on two things. Each thing could be a variable (eg: \texttt{a + b}), a constant (eg: \texttt{a + 5}) or some complicated expression (eg: \texttt{(2*a + 6) / (12 + b - x)} )\footnote{I contemplated leaving this closing parenthesis out to give you unresolved tension but I'm not quite \textit{that} evil.}.
...
The line \texttt{k = 2*k} takes the value of \texttt{k}, multiplies it by 2, then assigns that result back into the variable \texttt{k}. This is a \textit{crucial} concept in programming: the \texttt{=} symbol is \textbf{not} equality (a number is never equal to double itself unless the number is zero!), it is \textit{assignment}. Assignment takes what's on the right, evaluates it, then stores it into the variable on the left.

\pagebreak
\section{And Introduction to Git}
