...
You will observe that my notes break intrinsic documentation rules all the time. Sorry.
...
int main() {
	...
	scanf("%f", &x);
	// x = ??? uncomment this line and write your answer instead
	printf("y: %f\n", y);
	return 0;
}
\end{lstlisting}
...
Back to the task at hand, have a go at implementing Equation \ref{eq:fraction} and see if the result works. After you've had a go yourself read the examples and commentary below:

\begin{enumerate}
\item {\texttt{x = y/1-y}} // This won't work because \texttt{y/1} happens first
\item {\texttt{x = y/(1-y)}} // This one is correct
\end{enumerate}

As practice, implement as many of the following equations(what happens when you choose x to force a division by zero?):
...
For this one you need to know that C does \textit{not} include a ``to the power of'' operator. Implement $x^2$ as \texttt{x*x}:
\begin{equation}
y = x^2 + 2x.
\end{equation}
