...
\item Using a game engine's debug console (eg: accessed via the tilde \textasciitilde\ key)
\item High WPM touch typing
\item Use of keyboard shortcuts
\end{itemize}

are all excellent preparation for your programming adventure.
...
My support for Windows is lukewarm. Given you \textit{probably} grew up with Windows hopefully you can work out how to install and use the software below without much help. I don't have access to a Windows machine to test and debug installation of the recommendations.
...
Ok, I've never used a Mac but under the hood they are very similar to Linux (mostly the same command line, both Unix variants) so \texttt{gcc} should run fine...right??
...
\pagebreak
\subsection{Getting started with C in Ubuntu Linux - Optional}
...
\pagebreak
\subsection{Running Ubuntu With VirtualBox - Optional}\label{sec:virtualbox}
...
\item To get Ubuntu running you have a few options:
	\begin{enumerate}
		\item Download the pre-installed virtual machine image I created from: \url{http://chetesting.vk2dds.net/Ubuntu.ova}
