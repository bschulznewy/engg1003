...
All these tasks are based on pure mathematics which I \textit{believe} to be understood by a high school graduate who has been admitted to engineering (ie: 2 unit Advanced Mathematics or higher). Sequences and series are described in Section 7 of the NSW \underline{\href{https://educationstandards.nsw.edu.au/wps/portal/nesa/11-12/stage-6-learning-areas/stage-6-mathematics/mathematics-syllabus}{Mathematics 2/3 Unit syllabus}}.

These tasks may not create the most interesting programs in the world, but I need to set a solid foundation of knowledge among the whole cohort before we can tackle more complicated (and realistic) problems.
...
x_{n+1} = x_n + d.
\end{equation}
An arithmetic series can be defined by the initial value, $x_1$, and the difference, $d = x_n - x_{n-1}$.
\\ \\
For example, the sequence:
\begin{equation}
0, 2, 4, 6, ...
\end{equation}
is defined by $x_1 = 0$ and $d = 2$.
\\ \\
...
\\ \\
Check if your code works by comparing its output with a sequence calculated by hand.
\end{task}
...
k = \frac{x_{n+1}}{x_n}.
\end{equation}
Write a C program which is given two numbers, $x_1$ and $x_2$, and generates further numbers in their geometric sequence.
\\ \\
Your program should follow the following rules:
\begin{itemize}
\item It should print the initial values
...
\\ \\
Modify your geometric sequence program to calculate the sum of the first $n$ terms. The value of $n$ must be declared as a variable with a given initial value. Test your program with a variety of values for $n$. You should also test with a variety of different initial values, $x_1$ and $x_2$.
\\ \\
...
\begin{equation}
S_n = x_1\frac{1-k^{n+1}}{1-k}.
\end{equation}

Evaluate this formula \textit{by hand} to generate test values.
\end{task}

\begin{task}{Geometric Series Pt II}{}
If the value of the multiplier, $k$, in a geometric series is between (but not equal to) -1 and 1 then the series is said to \textit{converge}. This means that, as $n$ increases towards infinity, the sum of the series moves towards a fixed, finite, value. A geometric series which never stops growing is said to be \textit{divergent}.
\\ \\
Modify your geometric series program with the following rules:
\begin{itemize}
\item All numbers in the geometric sequence can be assumed to be positive
\item Given the values of $x_1$ and $x_2$ it decides if the series converges or diverges
...
Compare your program's output with the theoretical formula:

\begin{equation}
S_\infty = x_1 \frac{1}{1-k}
\end{equation}
\textbf{NB:} The precision limit can be implemented by testing the size of each term before it is added to the running sum.
\end{task}


\begin{task}{Second Order Sequences}{}
All previous sequences have been of \textit{first order}\footnote{Although not as compelling as the Galactic Empire the First Order still provides an entertaining plot for the Star Wars universe. Change my view.}. This means that a sequence value, $x_n$, only depends on $x_{n-1}$. A \textit{second order} sequence is one in which $x_n = f(x_{n-1}, x_{n-2})$. See footnote\footnote{This is read aloud as ``$x_n$ is a function of $x_{n-1}$ and $x_{n-2}$''}. Each value depends on the previous \textit{two} values.
\\ \\
As seen in lectures, the Fibonacci sequence is an example of a second order sequence. It follows the rule:
\begin{equation}\label{eq:fib}
x_n = x_{n-1} + x_{n-2}
\end{equation}
with the initial values of $x_1 = 1$ and $x_2 = 1$.
\\ \\
Equation \eqref{eq:fib} can be extended by introducing two arbitrary coefficients $b_1$ and $b_2$\footnote{Great, now I have the \textit{Bananas in Pajamas} theme in my head.}:
\begin{equation}
x_n = b_1x_{n-1} + b_2x_{n-2}.
\end{equation}
Although the Fibonacci sequence is restricted to integers assume any code written for this task requires \texttt{double} data types.
\\ \\
\textbf{The actual task:} Write a C program which is given initial values for $x_1$, $x_2$, $b_1$ and $b_2$ and calculates the resulting second order sequence.
...
\\ \\
Apart from the Fibonacci sequence, you may use the following test numbers:
\begin{lstlisting}[style=pseudo]
b1 = 0.2, b2 = 0.8
x1 = 1, x2 = 0.5
Output:
...
\begin{task}{Second Order Sequences Pt II (Advanced)}{}
The values of $b_1$ and $b_2$ will greatly influence the behaviour of a second order sequence. For example:
...
\item It may \textit{oscillate}
	\begin{itemize}
		\item ie: Wiggle above and below zero in a sinusoidal pattern
		\item This oscillation may converge to zero or diverge to infinity
	\end{itemize}
\end{itemize}
~\\
\textbf{Task: }Modify your second order sequence code to test for \textit{oscillation}. This can be done by calculating the \textit{difference} between subsequent numbers in the sequence and observing whether the difference changes sign (ie: goes positive and negative or is \textit{always} positive or negative).
\\ \\
...
\\ \\
If, after 20 iterations, no change of sign has been observed the program should print that no oscillation occurred and exit\footnote{This does not prove that oscillation will \textit{never} occur, just that if it does it takes a ``long time'' to be visible}.
\\ \\
...
-0.01520
\end{lstlisting}
This sequence oscillates (first difference is -1.5, then +0.6).
\\ \\
