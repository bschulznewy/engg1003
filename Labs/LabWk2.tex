...
These problems are \textit{not} meant to be executed in a C compiler. Read through them and write answers on paper (or Word etc). Solutions can be found on the last page of this document.
...
s^2 = \frac{\sum_{k=1}^{N}(X_k - mean(X))^2}{N-1}.
\end{equation}
...
\texttt{long}. An \texttt{int} only stores up to approx 2.1 billion so it can't be used
