...
The integers you type need to be separated by spaces (note that spaces are present inside the format string) and \texttt{$<$enter$>$} typed to end the line. The \texttt{scanf();} function won't process any data until an end of line is reached. Note that the order is \textit{crucial}.

The numbers you type will go into \texttt{x, y} and \texttt{z} in the the same order that the variable names are listed in \texttt{scanf();}.

You don't \textit{need} to separate the integers with spaces, \texttt{scanf();} supports incredibly advanced ``pattern matching'' using a system known as \textit{\href{https://xkcd.com/208/}{regular expressions}}. If you want to learn regular expressions be my guest, but it is \textit{well} beyond the scope of ENGG1003.
